...
- 2026-08-27（**形态被 #3c 取代**，见 [wiki 011](./011-caption-creative-directions.md)）：#4 开工前 grill 发现本票的 preprocess 形态有结构性 gap——「理解素材」与「生成方向」混在一次 normalize 里（无提炼产物），且 hooks/synopsis/setting/outline 四平行数组无方向绑定。#3c 重构为 caption（提炼稿，自动通过）→ creative（创意稿方向包 ×N，比较界面选定）两步；本票的 interview 两阶段、SKILL.md 提示词文件化、KnownError 等机制由 #3c 继承，preprocess 产物形态（inputStage + 四数组）作废。
